$(function() {
	...
	$("#btn-complete").click(function() {
		...
		$(".loading-mask").show();
		try {
			_hmt.push(['_trackEvent', "create_modify_qrcode_complete", "click", 1]);
		} catch (e) {}
		$.post(
			"/create/complete/",
			{
			...
			function(data) {
				...
				if (data.flag == 1) {
					...
					$(".create-login-mask").show();
					try {
						_hmt.push(['_trackEvent', "create_modify_login_ask", "click", 1]);
					} catch (e) {}
				}
			},
			"json"
		...
		$.ajaxFileUpload({
			...
			success: function(data, status) {
				...
				if (data.response == 1) {
				  ...
				  $('#logo-image-preview').attr("src", data.imagepath);
	...
	// 为logo添加拖放和resize的功能，拖放或者resize结束之后，会触发相应事件，并且返回最终的数据
	// 初始化时传入边界元素（此处是以.render-content元素为参照的），以及minWidth和maxWidth，注意，只需传入宽度
	$('#logo-image-preview').dragAndResize({
		boundary: $('.render-content'),
		minWidth: 50,
		maxWidth: 300
	}).on('drag-end', function(event, data) {
		console.log('drag-end', event, data);
	}).on('resize-end', function(event, data) {
		console.log('resize-end', event, data);
	});

	// //===================移动Logo位置=======================
	// $("#fuck").mousedown(function(e) {
	// 	e = e || window.event;
	// 	var offsetX = e.offsetX, offsetY = e.offsetY;
	// 	var height = $(this).height(), width = $(this).width();
	// 	if( (offsetX < 10 && offsetY < 10) || (offsetX > width - 10 && offsetY > height - 10)
	// 			|| (offsetX > width - 10 && offsetY < 10) || (offsetX < 10 && offsetY > height - 10)) {
	// 		console.log('resize---------')
	// 		return false;
	// 	}
	// 	var mousePosX,
